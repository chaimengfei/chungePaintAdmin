    ...
    <div v-loading="loading">
      ...
      <el-card style="margin-bottom: 20px;">
        ...
        <div style="padding: 0 20px 20px 20px;">
          <div style="margin-bottom: 16px; font-size: 18px; color: #F56C6C; font-weight: 600;">
            总金额：{{ totalSalesAmount }}元
          </div>
...
const industryColorMap = {
  'factory_advert': '#A8D8EA', // 淡蓝
  'factory_sculpt': '#AAE3B1', // 淡绿
  'worker': '#FFB6C1' // 淡粉
}
...
const totalSalesAmount = computed(() => {
  ...
  const total = stats.reduce((sum, item) => {
    return sum + parseFloat(item.order_amount || 0) / 100 // 分转元
  }, 0)
...
function renderSalesByIndustryChart() {
  ...
  const pieData = stats.map(item => {
    const industryName = industryMap[item.industry] || item.industry
    return {
      name: industryName,
      value: parseFloat(item.order_amount || 0) / 100, // 分转元
      itemStyle: {
        color: industryColorMap[item.industry] || '#E0E0E0'
      }
    }
  })
